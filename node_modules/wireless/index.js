...
var Wireless = function() {
    ...
    self._parseScan = function(scanResults) {
		...
		try {
                	network.quality = line.match(/Quality=([0-9]{1,2})\/100/)[1];
                	network.strength = line.match(/Signal level=(-?[0-9]{1,2})\/100/)[1];
		} catch (e) {
			try {
				network.quality = min(100, line.match(/Quality=([0-9]{1,2})/)[1]);
				network.strength = line.match(/Signal level=(-?[0-9]{1,2})/)[1];
			} catch (ef) {
				network.quality = 0, network.strength = 0;
			}
		}
	...
	if (self.currentlyScanning && self.grace > 0) {
